package org.openshift.drugstoredemo.rest;
...
import org.openshift.drugstoredemo.domain.Store;
import org.openshift.drugstoredemo.mongo.DBConnection;
...
@RequestScoped
@Path("/stores")
public class DrugStoreResource
{

	@Inject
	private DBConnection dbConnection;

	private DBCollection getDrugStoresCollection()
	{
	...
	public List<Store> getAllStores()
	{
		...
		DBCollection stores = this.getDrugStoresCollection();
		DBCursor cursor = stores.find();
		try
		{
	...
	@Produces("application/json")
	@Path("within")
	public List<Store> findStoresWithin(@QueryParam("lat1") float lat1, @QueryParam("lon1") float lon1,
			@QueryParam("lat2") float lat2, @QueryParam("lon2") float lon2)
	{
		...
		DBCursor cursor = stores.find(spatialQuery).limit(300);
		try
		{
			while (cursor.hasNext())
			{
